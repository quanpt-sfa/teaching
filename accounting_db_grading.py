...
def grade_one_pair(ans_t, stu_t, ans_schema, stu_schema, COL_TH=0.82):
    ...
    rows = []
    used = set()
    for i, (a_c, a_t) in enumerate(ans_cols):
        j = assign.get(i, None)
        if j is not None and j not in used:
            ...
            s_c = s_t = "—"; cs = 0.0; ok = False
        rows.append({
            "AnsTbl": ans_t, "AnsCol": a_c, "AnsType": a_t,
            "StuTbl": stu_t if j is not None and j < len(stu_cols) else "—",
            "StuCol": s_c,   "StuType": s_t,
            "Cos": cs, "Match": ok
        })
    return rows
...
def phase2_one(ans_tbl, stu_tbl, ans_schema, stu_schema):
    ...
    if stu_tbl is None:                    # không tìm được bảng SV
    ...
    for i, (cA, tA) in enumerate(ans_cols):
        ...
        for j, (cS, tS) in enumerate(stu_cols):
            ...
            if canonical(cA) == canonical(cS):           # trùng/alias
                ...
                hit = True
                break
        if hit is None:                                  # sẽ xử lý ở bước 2
            rows.append([ans_tbl, cA, tA, None, None, None, None, None])

    # ---------- BƯỚC 2 – tính 70 % cosine + 30 % token ----------
    #   chỉ áp dụng cho cột CHƯA match
    needA = [r for r in rows if r[6] is None]
    needB = [(j, *stu_cols[j]) for j in range(len(stu_cols)) if j not in matched_idx_stu]
    if needA and needB:
        vecA = np.stack([get_vec(f"{ans_tbl}.{r[1]} ({r[2]})") for r in needA])
        vecB = np.stack([get_vec(f"{stu_tbl}.{c} ({t})") for _,c,t in needB])
        # Ensure 2D for single vector case
        vecA = np.atleast_2d(vecA)
        vecB = np.atleast_2d(vecB)
        cos = vecA @ vecB.T
        # normalize each row of vecA and each row of vecB
        cos /= (np.linalg.norm(vecA, axis=1)[:, None] + 1e-8)
        cos /= (np.linalg.norm(vecB, axis=1)[None, :] + 1e-8)

        # token overlap
        tok   = np.zeros_like(cos)
        for i, rA in enumerate(needA):
            cA = rA[1]
            for k, nb in enumerate(needB):
                cS = nb[1]
                tok[i,k] = fuzz.token_set_ratio(canonical(cA), canonical(cS)) / 100.0

        score = 0.70 * cos + 0.30 * tok
        cost  = -score
        r_idx, c_idx   = linear_sum_assignment(cost)

        for i,j in zip(r_idx,c_idx):
            if score[i, j] <= 0:                       # không đủ điểm
                continue
            idx_stu, cS, tS = needB[j]
            cA, tA = needA[i][1:3]
            ok     = score[i, j] >= 0.82 and same_type(tA, tS, cA, cS)
            rows_id = rows.index(needA[i])             # vị trí dòng cần cập nhật
            rows[rows_id] = [ans_tbl, cA, tA, stu_tbl, cS, tS,
                              float(score[i, j]), ok]

    # Điền lại các ô chưa ghép
    for r in rows:
        if r[6] is None:                               # chưa gán ở bước 2
            r[3:] = ["—", "—", "—", 0.0, False][1:]    # StuTbl,…,Match
