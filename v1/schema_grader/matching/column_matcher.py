from ..utils.fuzzy import fuzzy_eq, smart_token_match
...
def phase2_one(ans_tbl, stu_tbl, ans_schema, stu_schema):
    ...
    # Bước 1: name-match trực tiếp và smart token match
    matched_idx_stu = set()
    rows = []
    for i, (cA, tA) in enumerate(ans_cols):
        ...
        for j, (cS, tS) in enumerate(stu_cols):
            ...
            # Kiểm tra exact match hoặc smart token match
            if canonical(cA) == canonical(cS) or smart_token_match(cA, cS) >= 80:
    ...
    if needA and needB:
        # Chuyển tên cột thành vectors - chỉ dùng tên đã chuẩn hóa
        ...
        for i,j in zip(r_idx,c_idx):
            ...
            # Điều kiện matching:
            # 1. cosine >= 0.75 HOẶC
            # 2. token_set_ratio = 100% HOẶC
            # 3. smart_token_match >= 80
            # VÀ phải tương thích kiểu dữ liệu
            token_score = smart_token_match(cA, cS)
            ok = (cos[i, j] >= 0.75 or 
                 fuzz.token_set_ratio(canonical(cA), canonical(cS)) == 100 or
                 token_score >= 80) \
                and same_type(tA, tS, cA, cS)
            
            rows_id = rows.index(needA[i])
            final_score = max(float(cos[i, j]), token_score / 100) 
...
def match_all_pairs(ans_schema, stu_schema, mapping, COL_TH=0.82):
    ...
    # Áp dụng alias mapping cho tên bảng sinh viên
    stu_schema_mapped = {}
    for tbl, info in stu_schema.items():
        tbl_mapped = mapping.get(tbl.lower(), tbl)  # Sử dụng mapping để map tên bảng
        stu_schema_mapped[tbl_mapped] = info
    
    ans_tbls = list(ans_schema.keys())
    stu_tbls = list(stu_schema_mapped.keys())  # Dùng tên bảng đã được map
    
    # Tạo items với tên bảng đã được map
    ans_items = [(t, c, d) for t, m in ans_schema.items() for c, d in m['cols']]
    stu_items = [(t, c, d) for t, m in stu_schema_mapped.items() for c, d in m['cols']]
    
    m, n = len(ans_items), len(stu_items)
    if m == 0 or n == 0:
        return [], 0.0, [], [], None
    
    # Convert column names to vectors using col_vec
    vA = np.stack([col_vec(it[1]) for it in ans_items])
    vB = np.stack([col_vec(it[1]) for it in stu_items])
    sim = vA @ vB.T
    sim /= np.linalg.norm(vA, axis=1)[:, None]
    sim /= np.linalg.norm(vB, axis=1)[None, :]
